...
{
    class IceMagicBehavior : IAttackBehavior
    {
        private string attackName = "Blizzard";
        ...
        {
            int damage;
            if (target.Stats.ICE_RES >= 2)
            {
                damage = 0;
            }
            else
            {
                damage = (int)Math.Floor(attacker.Stats.MAG_ATK) - ((int)Math.Floor(target.Stats.MAG_DEF) * (int)Math.Floor(target.Stats.ICE_RES));
